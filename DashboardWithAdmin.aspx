<%@ Page Title="" Language="C#" MasterPageFile="~/Assignment.Master" AutoEventWireup="true" CodeBehind="DashboardWithAdmin.aspx.cs" Inherits="Wapping_time.DashboardWithAdmin" %>
        ...
        function previewCourseImage(input) {
            ...
            if (input.files && input.files[0]) {
                ...
                reader.onload = function (e) {
                    ...
                    document.getElementById("<%= courseFileUpload.ClientID %>").style.display = "none";
                };
    ...
    <div id="globalDiv">
        ...
        <div id="bottomDiv">
            ...
            <div id="bottomBottomDiv">
                ...
                <h4 id="noCourseMsg" style="text-align: center;" runat="server" visible="false">You haven't created any courses!</h4>

                <asp:Repeater ID="CourseRepeater" runat="server" DataSourceID="SqlDataSource1" OnItemCommand="Repeater1_ItemCommand">
                    <ItemTemplate>
                        <div class="courseBox">
                            <div class="contentInCourseBox">
                                <img 
                                    src='<%# string.IsNullOrEmpty(Eval("courseImage").ToString()) ? "/Images/penguin.png" : Eval("courseImage") %>' 
                            ...
                            <div class="contentInCourseBox text-description" style="background-color:rgb(255 255 255 / var(--tw-bg-opacity, 1)); display:flex; flex-direction:column; justify-content:space-between">
                                ...
                                <div style="width:100%; display:flex; justify-content:flex-end ">
                                    <asp:Button ID="continueBtn" runat="server" Text="Button" CssClass="createCourseButton"/>
                                </div>
                            </div>
                        </div>
                    </ItemTemplate>
                </asp:Repeater>
                <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:ReadCardDB %>" SelectCommand="SELECT * FROM [Course]" DeleteCommand="DELETE FROM [Course] WHERE [CourseID] = @CourseID" InsertCommand="INSERT INTO [Course] ([UserID], [CourseName], [Description], [CourseCreatedDate], [CourseImage], [CourseCategory]) VALUES (@UserID, @CourseName, @Description, GETDATE(), @CourseImage, @CourseCategory)" UpdateCommand="UPDATE [Course] SET [UserID] = @UserID, [CourseName] = @CourseName, [Description] = @Description, [CourseCreatedDate] = @CourseCreatedDate, [CourseImage] = @CourseImage WHERE [CourseID] = @CourseID">
                    <DeleteParameters>
    ...
    <div id="createCourseModal" class="modal-overlay">
        <div class="modal-box">
            ...
            <div class="form-grid">
                ...
                <span>:</span>
                <asp:FileUpload ID="courseFileUpload" ClientIDMode="Static" accept="image/*" runat="server" onchange="previewCourseImage(this)" CssClass="h-createCourseBox"/>
                <div id="imagePreviewWrapper" class="image-preview-wrapper" style="display:none;">
                    <button type="button" class="remove-image-btn" onclick="removeCourseImage()">×</button>

                    <img id="courseImagePreview" src=null draggable="false" ondragstart="return false;" class="image-preview" />
                </div>
                ...
                <span>:</span>
                <asp:TextBox ID="descriptionTxt" runat="server" TextMode="MultiLine" CssClass="input-box textarea-box" />

            </div>
            
            <div class="button-row">
                <asp:Button ID="saveCourseBtn" runat="server" Text="Save Course" OnClick="SaveCourseBtn_Click" />
                <asp:Button ID="cancelBtn" runat="server" Text="Cancel" OnClick="CancelBtn_Click" OnClientClick="closeCreateCoursePopup()" />
            </div>
        </div>
    </div>
</asp:Content>
